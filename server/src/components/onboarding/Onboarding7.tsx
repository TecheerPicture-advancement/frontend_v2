...
const Onboarding7: React.FC = () => {
    return(
        <div className="w-full h-[826px] relative overflow-hidden bg-light-gradient dark:bg-dark-gradient">
  <h1 className="absolute left-[1100px] top-[360px] text-[clamp(3rem,9vw,13rem)] font-bold text-right font-PR_BO dark:text-green-Light">
    Upload
  </h1>
  <div className="flex flex-col justify-start items-end w-[843px] absolute left-[-23px] top-24">
      <h1 className="self-stretch flex-grow-0 flex-shrink-0 w-[843px] text-[200px] font-bold text-right font-PR_BO dark:text-green-Light">
        Intagram
      </h1>
      <br />
    <div className="flex flex-col justify-start items-end self-stretch flex-grow-0 flex-shrink-0 relative gap-[31px] px-[100px]">
    <p className="font-PR_BO dark:text-green-Light text-[clamp(1rem,2vw,1.5rem)]">
        <span className="self-stretch flex-grow-0 flex-shrink-0 w-[643px] text-xl text-right font-PR_M dark:text-green-Light font=PR_R">
          TecheerPicture에서 제작한 사진과 비디오로
        </span>
        <br />
        <span className="self-stretch flex-grow-0 flex-shrink-0 w-[643px] text-xl text-right font-PR_M dark:text-green-Light">
          인스타그램에 바로 업로드 해보세요
        </span>
      </p>
      <button className="flex justify-center items-center gap-4 bg-green-Normal px-6 py-3 rounded-xl mt-4">
      <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left font-PR_M text-black">
          제작하러가기
        </p>
        <Arrow/>
      </button>
